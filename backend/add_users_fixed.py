...
def add_users(tx):
    for i, user in enumerate(users):
        ...
        event_id = "otcom-1"
        tx.run("""
            MERGE (u:users {user_id: $user_id})
            SET u.name = $name, u.skills = $skills, u.interests = $interests, u.networking_goals = $networking_goals
            MERGE (e:events {event_id: $event_id})
            SET e.event_name = 'Otcom'
            MERGE (u)-[:ATTENDING]->(e)
        """, user_id=user_id, name=user['name'], skills=user['skills'], interests=user['interests'], networking_goals=user['networking_goals'], event_id=event_id)
    print("Added 20 tech-focused users to Neo4j (user_id unique, linked to Otcom event).")
...
driver.close()
print("Run 'python count_users.py' to verify :users count >20!")
